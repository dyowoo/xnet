...
func NewTimeWheel(name string, interval int64, scales int, maxCap int) *TimeWheel {
	...
	xlog.InfoF("Init timerWheel name = %s is done!", tw.name)
	return tw
}
...
func (tw *TimeWheel) addTimer(tID uint32, t *Timer, forceNext bool) error {
	defer func() {
		if err := recover(); err != nil {
			xlog.ErrorF("addTimer function err : %s", err)
		}
	}()

	// 得到当前的超时时间间隔(ms)毫秒为单位
	delayInterval := t.units - UnixMilli()
...
func (tw *TimeWheel) AddTimeWheel(next *TimeWheel) {
	...
	xlog.InfoF("add timerWhell[%s]'s next [%s] is success!", tw.name, next.name)
}
...
func (tw *TimeWheel) Run() {
	go tw.run()
	xlog.InfoF("timerwheel name = %s is running...", tw.name)
}
...
func (tw *TimeWheel) GetTimerWithIn(duration time.Duration) map[uint32]*Timer {
	...
	//1 找到最底层时间轮
	leaflet := tw
	for leaflet.nextTimeWheel != nil {
		leaflet = leaflet.nextTimeWheel
	}

	leaflet.Lock()
	defer leaflet.Unlock()
	//返回的Timer集合
	...
	//取出当前时间轮刻度内全部Timer
	for tID, timer := range leaflet.timerQueue[leaflet.curIndex] {
		if timer.units-now < int64(duration/1e6) {
			//当前定时器已经超时
			timerList[tID] = timer
			//定时器已经超时被取走，从当前时间轮上 摘除该定时器
			delete(leaflet.timerQueue[leaflet.curIndex], tID)
		}
	}
